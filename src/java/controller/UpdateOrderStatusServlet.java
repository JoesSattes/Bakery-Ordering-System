    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            try {
                ...
                pstm = conn.prepareStatement("UPDATE order_item SET status = ? WHERE item_no=? AND shopping_basket_id=?");
                pstm.setString(1, status);
                pstm.setInt(2, Integer.parseInt(item_no));
                pstm.setInt(3, Integer.parseInt(shopping_basket_id));
                pstm.executeUpdate();
